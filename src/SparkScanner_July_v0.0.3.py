...
class SparkMidiApp:
    ...
    def build_spark_packet(self, cmd_bytes, force_10_zeros=False):
        ...
        force_10_zeros: True only for Spark GO.
        """
        ...
        # 10 zeros only for GO, 9 zeros for Mini/40/2.
        zeros_count = 10 if force_10_zeros else 9
        zeros = [0x00] * zeros_count
        
        # Total packet length: 7 header bytes (magic + length) + zeros count + payload length
    ...
    async def send_to_spark(self, btn_id):
        if self.spark_client and self.spark_client.is_connected:
            if self.is_go_model:
                # For Spark GO: use command [btn_id, 0x01, 0x38, 0x00, 0x00, btn_id] and 9 zeros
                ...
                # For Spark MINI: use 9 zeros and command 03 38
                ...
                try:
                    ...
                    if target:
                        ...
                        if self.is_go_model or self.is_mini_model:
                            await self.spark_client.write_gatt_char(WRITE_UUID, self.build_spark_packet([0x00, 0x02, 0x2f], force_10_zeros=False), response=False)
                            await asyncio.sleep(0.1)
                            await self.spark_client.write_gatt_char(WRITE_UUID, self.build_spark_packet([0x00, 0x02, 0x23], force_10_zeros=False), response=False)
                        else:
